...
function handleTaskCompletion(
  ...
  let totalTask = (document.getElementById(taskAssignedId).innerText =
    assignNumber - 1);
  ...
  // activity log
  let IssueText = document.getElementById(textContainerId).innerText;
  let pera = document.createElement("p");
  pera.innerText =
    "You have completed the task " + IssueText + " " + getCurrentTime();
  pera.classList.add("activity-log");

  document.getElementById(logContainerId).appendChild(pera);
  alert("Board Update SuccessFull " + IssueText);
